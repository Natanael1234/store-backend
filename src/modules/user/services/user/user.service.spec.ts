...
describe('UserService', () => {
  ...
  describe('create', () => {
    ...
    describe('name', () => {
      ...
      it('should validate if name lenght is valid', async () => {
        ...
        const expectedData = [
          { id: 1, name: shortName, email: usersData[0].email },
          { id: 2, name: longName, email: usersData[1].email },
        ];
  ...
  describe('update', () => {
    ...
    it('should fail when thre are multiple invalid fields', async () => {
      ...
      const usersBefore = await userService.findAll();
      async function fn() {
        await userService.update(2, { name: newName, email: newEmail });
      }
      await expect(fn()).rejects.toThrow(UnprocessableEntityException);
      expect(usersBefore).toEqual(await userService.findAll());
      await expect(fn()).rejects.toThrow('Unprocessable Entity Exception');
      try {
        ...
        expect(error.response).toEqual({
          error: UnprocessableEntityException.name,
          message: { name: NameMessage.MIN_LEN, email: EmailMessage.INVALID },
          statusCode: HttpStatus.UNPROCESSABLE_ENTITY,
        });
      }
    });

    // TODO: create list errors ???
    describe('id', () => {
    ...
    describe('email', () => {
      ...
      it('should validate if email lenght is valid', async () => {
        ...
        const expectedData = [
          { id: 1, name: usersData[0].name, email: usersData[0].email },
          { id: 2, name: usersData[1].name, email: longEmail },
        ];
        const createdUsers = [
          await userService.create(userCreationData[0]),
          await userService.create(userCreationData[1]),
        ...
        try {
          const updatedUsers = [
            await userService.update(2, { email: longEmail }),
          ];
          const users = await userRepo.find();
