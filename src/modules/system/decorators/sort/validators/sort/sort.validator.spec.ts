...
describe('IsSorting', () => {
  ...
  describe('array items', () => {
    ...
    it('should not validate when receives repeated array item (same column)', async () => {
      class dTO {
        @IsSorting(TestEnum)
        test: TestEnum[];
      }
      const dto = plainToInstance(dTO, {
        test: [TestEnum.value3, TestEnum.value2, TestEnum.value4],
      });
      const errors = await validate(dto);
      expect(errors).toHaveLength(1);
      expect(errors[0].property).toEqual('test');
      expect(errors[0].constraints.isSorting).toEqual(SortMessage.REPEATED);
    ...
    it('should not validate when receives repeated array item (same column and direction)', async () => {
      class dTO {
        @IsSorting(TestEnum)
        test: TestEnum[];
      }
      const dto = plainToInstance(dTO, {
        test: [TestEnum.value3, TestEnum.value2, TestEnum.value3],
      });
      const errors = await validate(dto);
      expect(errors).toHaveLength(1);
      expect(errors[0].property).toEqual('test');
