...
describe('ProductImageService.bulkSave (metadata.description)', () => {
  ...
  describe('create', () => {
    it('should accept create image when metadata.description has maximum allowed length', async () => {
      ...
      const ret = await productImageService.bulkSave(productId1, files, {
    ...
    it('should reject create image when metadata.description is longer than allowed', async () => {
      ...
      try {
        ...
        expect(ex.getResponse()).toEqual({
          ...
          message: { metadatas: DescriptionMessage.MAX_LEN },
    ...
    it('should reject create image when metadata.description is number', async () => {
      ...
      const fn = () =>
        productImageService.bulkSave(productId1, imageFiles, {
          metadatas: [{ description: 1 as unknown as string, imageIdx: 0 }],
        });
      ...
      try {
        ...
        expect(ex.getResponse()).toEqual({
          error: ExceptionText.UNPROCESSABLE_ENTITY_EXCEPTION,
          message: { metadatas: DescriptionMessage.INVALID },
          statusCode: HttpStatus.UNPROCESSABLE_ENTITY,
        });
      }
    ...
    it('should reject create image when metadata.description is boolean', async () => {
      ...
      const fn = () =>
        productImageService.bulkSave(productId1, imageFiles, {
          metadatas: [{ description: true as unknown as string, imageIdx: 0 }],
        });
    ...
    it('should reject create image when metadata.description is array', async () => {
      ...
      const fn = () =>
        productImageService.bulkSave(productId1, imageFiles, {
          metadatas: [{ description: [] as unknown as string, imageIdx: 0 }],
        });
  ...
  describe('update', () => {
    it('should accept update image when metadata.description has maximum allowed length', async () => {
      ...
      const ret = await productImageService.bulkSave(products[0].id, null, {
        metadatas: [{ description, imageId: products[0].images[0].id }],
      });
    ...
    it('should accept update image when metadata.description is null', async () => {
      ...
      const ret = await productImageService.bulkSave(products[0].id, null, {
        metadatas: [{ description: null, imageId: products[0].images[0].id }],
      });
      const expectedResults = [
