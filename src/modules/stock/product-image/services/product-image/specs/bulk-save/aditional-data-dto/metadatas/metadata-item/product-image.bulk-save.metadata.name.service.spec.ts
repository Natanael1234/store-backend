...
describe('ProductImageService.bulkSave (metadata.name)', () => {
  ...
  describe('create', () => {
    it('should accept create image when metadata.name has maximum allowed length', async () => {
      ...
      const retImages = await productImageService.bulkSave(productId1, files, {
        metadatas: [{ name, imageIdx: 0 }],
      });
    ...
    it('should reject create image when metadata.name is longer than allowed', async () => {
      ...
      try {
        ...
        expect(ex.getResponse()).toEqual({
          ...
          message: { metadatas: NameMessage.MAX_LEN },
    ...
    it('should reject create image when metadata.name is number', async () => {
      ...
      const fn = () =>
        productImageService.bulkSave(productId1, imageFiles, {
          metadatas: [{ name: 1 as unknown as string, imageIdx: 0 }],
        });
      ...
      try {
        ...
        expect(ex.getResponse()).toEqual({
          error: ExceptionText.UNPROCESSABLE_ENTITY_EXCEPTION,
          message: { metadatas: NameMessage.INVALID },
          statusCode: HttpStatus.UNPROCESSABLE_ENTITY,
        });
      }
    ...
    it('should reject create image when metadata.name is boolean', async () => {
      ...
      const fn = () =>
        productImageService.bulkSave(productId1, imageFiles, {
          metadatas: [{ name: true as unknown as string, imageIdx: 0 }],
        });
    ...
    it('should reject create image when metadata.name is array', async () => {
      ...
      const fn = () =>
        productImageService.bulkSave(productId1, imageFiles, {
          metadatas: [{ name: [] as unknown as string, imageIdx: 0 }],
        });
  ...
  describe('update', () => {
    it('should accept update image when metadata.name has maximum allowed length', async () => {
      ...
      const ret = await productImageService.bulkSave(products[0].id, null, {
        metadatas: [{ name, imageId: products[0].images[0].id }],
      });
    ...
    it('should accept update image when metadata.name is null', async () => {
      ...
      const ret = await productImageService.bulkSave(products[0].id, null, {
        metadatas: [{ name: null, imageId: products[0].images[0].id }],
      });
    ...
    it('should accept update image when metadata.name is undefined', async () => {
      ...
      const ret = await productImageService.bulkSave(products[0].id, null, {
        metadatas: [{ name: undefined, imageId: products[0].images[0].id }],
      });
