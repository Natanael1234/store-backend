...
describe('ProductImageService.bulkSave (metadata.imageId)', () => {
  ...
  it('should accept update one image of a product', async () => {
    ...
    const response = await productImageService.bulkSave(products[0].id, null, {
      metadatas: [
        {
  ...
  it('should accept update metadata.imageId when imageFiles is null', async () => {
    const [product1, product2] = await testBuildProductImageUpdateScenario();
    const ret3 = await productImageService.bulkSave(product1.id, null, {
      metadatas: [{ name: 'Image 1 b', imageId: product1.images[0].id }],
    });
  ...
  it('should accept update metadata.imageId when imageFiles is undefined', async () => {
    const [product1, product2] = await testBuildProductImageUpdateScenario();
    const ret3 = await productImageService.bulkSave(product1.id, undefined, {
      metadatas: [{ name: 'Image 1 b', imageId: product1.images[0].id }],
    });
    // images in the cloud storage
    expect(Client._getBucketsSnapshot()).toBeDefined();
  ...
  it('should accept update maximum allowed metadata items', async () => {
    ...
    const createMetadatas = imageFiles.map((imageFilke, imageIdx) => {
      const imageNumber = imageIdx + 1;
      const name =
        imageNumber < 10 ? `Image 0${imageNumber}` : `Image ${imageNumber}`;
      return { imageIdx, main: imageIdx == 0, active: true, name };
    });
    ...
    const metadatasForUpdate = createMetadatas.map((metadata, imageIdx) => {
      return { imageId: retCreate[imageIdx].id, name: metadata.name + ' b' };
    });
    const imagesBefore = await getImages();

    const retUpdate = await productImageService.bulkSave(productId1, null, {
      metadatas: metadatasForUpdate,
    });
  ...
  it('should reject when metadata.imageId is boolean', async () => {
    ...
    const fn = () =>
      productImageService.bulkSave(product.id, imageFiles, {
        metadatas: [{ active: true, imageId: true as unknown as string }],
      });
    ...
    try {
      ...
      expect(ex.response).toEqual({
        ...
        message: { metadatas: ImagesMetadataMessage.IMAGE_ID_INVALID },
  ...
  it('should reject when metadata.imageId is invalid string', async () => {
    ...
    const fn = () =>
      productImageService.bulkSave(product.id, imageFiles, {
        metadatas: [{ active: true, imageId: 'not-a-valid-uuid' }],
      });
    const imagesBefore = await productImageRepo.find({ withDeleted: true });
    await expect(fn()).rejects.toThrow(UnprocessableEntityException);
    try {
  ...
  it('should reject when metadata.imageId is array', async () => {
    ...
    const fn = () =>
      productImageService.bulkSave(product.id, imageFiles, {
        metadatas: [{ active: true, imageId: [] as unknown as string }],
      });
    const imagesBefore = await productImageRepo.find({ withDeleted: true });
    await expect(fn()).rejects.toThrow(UnprocessableEntityException);
    try {
  ...
  it('should reject when metadata.imageId references a inexistent image', async () => {
    ...
    const fn = () =>
      productImageService.bulkSave(product.id, [file], {
        metadatas: [
  ...
  it('should reject when metadata contains image id of a different product', async () => {
    ...
    const fn = () =>
      productImageService.bulkSave(products[1].id, null, {
        metadatas: [
          {
            imageId: products[0].images[0].id,
            main: !!products[0].images[0].main,
  ...
  it('should reject when metadatas cointains multiple items with the same iamgeId', async () => {
    const products = await testBuildProductImageUpdateScenario();
    const imagesBefore = await productImageRepo.find();
    const fn = () =>
    ...
    try {
      ...
      expect(ex.response).toEqual({
        error: ExceptionText.UNPROCESSABLE_ENTITY_EXCEPTION,
        message: { metadatas: ImagesMetadataMessage.IMAGE_ID_DUPLICATED },
        statusCode: HttpStatus.UNPROCESSABLE_ENTITY,
      });
    }
    const imagesAfter = await productImageRepo.find();
