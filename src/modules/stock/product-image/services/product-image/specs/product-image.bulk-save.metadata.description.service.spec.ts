...
describe('ProductImageService.bulkSave (metadata.description)', () => {
  ...
  describe('create', () => {
    ...
    it('should accept create image when metadata.description is undefined', async () => {
      ...
      const ret = await productImageService.bulkSave(productId1, [
        { description: undefined, file: file1 },
      ]);
    ...
    it('should reject create image when metadata.description is longer than allowed', async () => {
      ...
      try {
        ...
        expect(ex.getResponse()).toEqual({
          ...
          message: DescriptionMessage.MAX_LEN,
    ...
    it('should reject create image when metadata.description is number', async () => {
      ...
      const fn = () =>
        productImageService.bulkSave(productId1, [
          { description: 1 as unknown as string, file: file1 },
        ]);
      ...
      try {
        ...
        expect(ex.getResponse()).toEqual({
          error: ExceptionText.UNPROCESSABLE_ENTITY,
          message: DescriptionMessage.INVALID,
          statusCode: HttpStatus.UNPROCESSABLE_ENTITY,
        });
      }
    ...
    it('should reject create image when metadata.description is array', async () => {
      ...
      const fn = () =>
        productImageService.bulkSave(productId1, [
          { description: [] as unknown as string, file: file1 },
        ]);
