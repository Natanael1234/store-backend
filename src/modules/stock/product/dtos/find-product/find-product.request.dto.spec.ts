...
import { FindProductRequestDTO } from './find-product.request.dto';
...
describe('FindProductsRequestDTO', () => {
  ...
  describe('multiple errors', () => {
    it('should fail with multiple errors', async () => {
      ...
      expect(errors[0].constraints).toEqual({ isEnum: ActiveMessage.INVALID });
      expect(errors[1].constraints).toEqual({ isEnum: DeletedMessage.INVALID });
    });
  });
});
