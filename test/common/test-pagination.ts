...
type CreateViaRepositotyCallback<T> = (quantity: number) => Promise<any>;
...
export function testPagination<T>(
  ...
  it('should return the first page with default page size when both page and pageSize are not defined', async () => {
    await createViaRepository(15);
    const [registers] = await getPagesFromRepository({ skip: 0, take: 12 });
  ...
  it('should return empty results when there is no registes saved', async () => {
    const ret = await getPagesFromAPI({ page: 1, pageSize: 12 }, HttpStatus.OK);
    expect(ret).toEqual({
      count: 0,
      page: 1,
      pageSize: 12,
  ...
  it('should return a specific page with specific size when both page and pageSize are specified', async () => {
    await createViaRepository(15);
    const [registers] = await getPagesFromRepository({ skip: 6, take: 3 });
    const ret = await getPagesFromAPI({ page: 3, pageSize: 3 }, HttpStatus.OK);
    expect(ret).toEqual({
      count: 15,
      page: 3,
      pageSize: 3,
  ...
  it('should return empty results when page is greater than the last page', async () => {
    await createViaRepository(15);
    const ret = await getPagesFromAPI({ page: 4, pageSize: 12 }, HttpStatus.OK);
    expect(ret).toEqual({
      count: 15,
      page: 4,
      pageSize: 12,
  ...
  describe('page', () => {
    it('should return first page when page parameter is equal to the minimum allowed page', async () => {
      await createViaRepository(15);
      const [registers] = await getPagesFromRepository({ skip: 0, take: 12 });
      const ret = await getPagesFromAPI({ page: 1 }, HttpStatus.OK);
      expect(ret).toEqual({
        count: 15,
        page: 1,
        pageSize: 12,
    ...
    it('should return another page when page parameter is greater than the first page', async () => {
      await createViaRepository(15);
      const [registers] = await getPagesFromRepository({ skip: 12, take: 12 });
      const ret = await getPagesFromAPI({ page: 2 }, HttpStatus.OK);
      expect(ret).toEqual({
        count: 15,
        page: 2,
        pageSize: 12,
    ...
    it('should return first page when page parameter is less than the minimum allowed page', async () => {
      await createViaRepository(15);
      const [registers] = await getPagesFromRepository({ skip: 0, take: 12 });
      const ret = await getPagesFromAPI({ page: 0 }, HttpStatus.OK);
      expect(ret).toEqual({
        count: 15,
        page: 1,
        pageSize: 12,
  ...
  describe('page size', () => {
    it('should return a page with the minimum allowed size when "pageSize" parameter is equal to the minimum allowed size', async () => {
      await createViaRepository(3);
      const [registers] = await getPagesFromRepository({ skip: 0, take: 1 });
      const ret = await getPagesFromAPI({ pageSize: 1 }, HttpStatus.OK);
      expect(ret).toEqual({
        count: 3,
        page: 1,
        pageSize: 1,
    ...
    it('should return a page with the maximum allowed size when "pageSize" parameter is equal to the maximum allowed size', async () => {
      await createViaRepository(41);
      const [registers] = await getPagesFromRepository({ skip: 0, take: 40 });
      const ret = await getPagesFromAPI({ pageSize: 40 }, HttpStatus.OK);
      expect(ret).toEqual({
        count: 41,
        page: 1,
        pageSize: 40,
    ...
    it('should return a page with the maximum allowed size when "pageSize" parameter is greater than the maximum allowed size', async () => {
      await createViaRepository(41);
      const [registers] = await getPagesFromRepository({ skip: 0, take: 40 });
      const ret = await getPagesFromAPI({ pageSize: 41 }, HttpStatus.OK);
      expect(ret).toEqual({
        count: 41,
        page: 1,
        pageSize: 40,
